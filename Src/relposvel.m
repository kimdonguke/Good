function [dgnss, stat] = relposvel(dgnss, obs, nav)
% ==================================================================================================
% RELPOS
...
y    = NaN(nr+nb, nf*2);        % un-differenced residuals
e    = NaN(nr+nb, 3*2);         % line of sight
...
y    = NaN(size(obs, 1), nf*2);
sat  = obs(:,3);
code = obs(:,29:33);
freq = NaN(size(obs, 1), nf*2);

for f = 1:nf
    freq(:,f) = code2freq(sat, code(:,f));
    pr = obs(:, 4+(f - 1));
    prr = obs(:,14+(f - 1));
    valid_pr = ~isnan(pr) & pr ~= 0; % code valid
    valid_dr = ~isnan(prr) & prr ~= 0; % doppler valid
    
    % residuals = observable - model
    y(valid_pr, f) = pr(valid_pr) - r(valid_pr) - dant(valid_pr, f);
    y(valid_dr, f+2) = prr(valid_dr) - r(valid_dr) - dant(valid_dr, f+2);
end
...
VAR_POS = 1^2; % initial variance of position [m^2]
VAR_VEL = 0.1^2;
VAR = [VAR_POS, VAR_VEL];
[dgnss] = initx(dgnss, dgnss.sol.rr(1:6), VAR, 1:6);
