...
% [12, 23, 28, 32, 39, 41:43, 47, 51:52, 65, 76:77, 93, 96, 98]
Point = [rtsStations.LAT_deg,rtsStations.LON_deg];
DT = delaunayTriangulation(Point([1:11, 13:22, 24:27, 29:31, 33:38, 40, 44:46, 48:50, 53:64, 66:75, 78:92, 94:95, 97, 99],2),...
    Point([1:11, 13:22, 24:27, 29:31, 33:38, 40, 44:46, 48:50, 53:64, 66:75, 78:92, 94:95, 97, 99],1));

% Point = [rtsStations.LAT_deg,rtsStations.LON_deg];
% DT = delaunayTriangulation(Point(:,2),Point(:,1));


% 1. DT 객체에서 경도(X)와 위도(Y) 데이터 추출
lon = DT.Points(:, 1);
lat = DT.Points(:, 2);

% 2. 삼각망을 구성하는 모든 선분(Edge)의 양끝점 인덱스 추출
% (중복된 선을 제외하고 순수한 연결선만 뽑아줍니다)
% E = edges(DT);
% 
% % 3. geoplot으로 한 번에 그리기 위한 데이터 재구성 (NaN 기법)
% % 선분(시작점-끝점) 사이에 NaN을 넣어주면 지도가 선을 이어그리지 않고 끊어서 그립니다.
...
% % 4. 지도 위에 시각화 (geoplot)
% figure;
% % 지형도 배경 설정 (원하는 배경으로 변경 가능: 'streets', 'satellite', 'colorterrain' 등)
% geobasemap('topographic'); 
% hold on;
% 
% % 삼각망의 선분 그리기 (geoplot은 위도, 경도 순서로 입력!)
% geoplot(lat_edges, lon_edges, 'k-', 'LineWidth', 1.2);

% 관측소(노드) 위치에 점 찍기
% geoplot(lat, lon,'ks','LineWidth',1,  'MarkerFaceColor', 'y', 'MarkerSize', 5);
% geoplot(Point([32,41,47,52,76,77],1), Point([32,41,47,52,76,77],2),'bo','MarkerFaceColor','b','MarkerSize',5);
% geoplot(CORS_TruePos_LLA(:,1), CORS_TruePos_LLA(:,2),'bo','MarkerFaceColor','b','MarkerSize',5);



% title('관측소 기준 들로네 삼각망 시각화');
% hold off;
...
% 1. 테스트 지점(초록색 및 파란색 원)들의 위도, 경도 좌표 통합
test_lat = [Point([12, 23, 28, 32, 39, 41:43, 47, 51:52, 65, 76:77, 93, 96, 98], 1); CORS_TruePos_LLA(:, 1)];
test_lon = [Point([12, 23, 28, 32, 39, 41:43, 47, 51:52, 65, 76:77, 93, 96, 98], 2); CORS_TruePos_LLA(:, 2)];
...
geoplot(gx, test_lat(1:17), test_lon(1:17), 'k^', 'LineWidth', 0.5, 'MarkerFaceColor', 'g', 'MarkerSize', 5);

% CORS 상설감시국 위치에 파란색 원 찍기 (18번 이후)
geoplot(gx, test_lat(18:end), test_lon(18:end), 'k^', 'LineWidth', 0.5, 'MarkerFaceColor', 'b', 'MarkerSize', 5);
legend('','국토지리정보원 기준국','국토지리정보원 상시관측소','타기관 상시관측소')
