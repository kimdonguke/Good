...
- **타이틀 규약 (2026-07-27 확정)**: 제목 = 주제 + 조건 식별자(망 이름·시나리오·기선 상한)만.
  산출 통계(면적·평균·만족율·미달 점수 등)는 제목에서 제외 — 상세 수치는 콘솔/요약 txt 로,
  개소 수는 legend 가 담당(`기준국 (%d개소)` 형식). 예외: PPT 단계별 figure(`ppt_*`)는 교육용 유지.
